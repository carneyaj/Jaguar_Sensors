...
def analysis(data,samplerate):
	...
	db_melspec = melspec(waveform, samplerate)
	new_indices = np.array([ACI_New(db_melspec), BI_New(db_melspec), AEI_New(db_melspec), ADI_New(db_melspec)], dtype = params.out_dtype)



	timestamp = np.array(datetime.now().timestamp())
	bioindices = np.array([ACI,BI,AEI,ADI], dtype = params.out_dtype)
	return timestamp, bioindices, new_indices, scores
...
def melspec(data, sr, n_fft=params.n_fft, hop_length=params.hop_length, n_mels=params.mels, fmin=params.fmin, fmax=params.fmax): #note the min and max frequencies; these can be changed.
	#data = data/np.max(data) #normalize the amplitude
	data = data * 20 #apply a fixed 20x (digital) gain - you can experiment with this.
	melspec = librosa.feature.melspectrogram(y=data,sr=sr,power=1, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels, fmin=fmin, fmax=fmax) #generate a mel scale spectrogram
	melspec = melspec/np.max(melspec) #normalize
	db_melspec = librosa.amplitude_to_db(melspec, ref=1)
	return db_melspec

def ACI_New(db_mel):
	mels = db_mel.shape[0]
	aci = np.sum(np.var(db_mel, axis=1))
	norm = mels
	return aci / norm

def BI_New(db_mel):
	mels = db_mel.shape[0]
	#BI_mean = 10 * np.log10(np.mean(mel ** 2, axis=1))
	BI_mean = np.mean(db_mel, axis=1)
	BI_mean_norm = BI_mean - np.min(BI_mean)
	#print(BI_mean_norm.astype(int))
	area = np.sum(BI_mean_norm) / mels
	return area


def AEI_New(db_mel, db_threshold=-30, bins=10):
	mels = db_mel.shape[0]
	db_mel_bands = [db_mel[int(mels / bins * k):int(mels / bins * (k+1)),] for k in range(bins)]
	#values = [np.sum(db_mel_bands[k]>db_threshold) / float(db_mel_bands[k].size) for k in range(bins)]
	means = [np.mean(db_mel_bands[k]) for k in range(bins)]
	values = [np.sum(db_mel_bands[k] > (means[k] + 10)) / float(db_mel_bands[k].size) for k in range(bins)]
	#values = [np.var(db_mel_bands[k]) for k in range(bins)]
	#print(values)
	return gini(values)

def ADI_New(db_mel, db_threshold=-50, bins=10):
	mels = db_mel.shape[0]
	db_mel_bands = [db_mel[int(mels / bins * k):int(mels / bins * (k+1)),] for k in range(bins)]
	#values = [np.sum(db_mel_bands[k]>db_threshold) / float(db_mel_bands[k].size) for k in range(bins)]
	#values = [np.var(db_mel_bands[k]) for k in range(bins)]
	...
	values = [np.sum(db_mel_bands[k] > (means[k] + 10)) / float(db_mel_bands[k].size) for k in range(bins)]
	values = [value for value in values if value != 0]
	return np.sum([-i/ np.sum(values) * np.log(i / np.sum(values))  for i in values]) #Shannon entropy
